...
#--------
# Load COPD HES data
COPD_HES <- read_xlsx("C:/Alex Harley/Audit_2023_onwards/2022-2023/COPD/Data/rawData/NIC-122140-L1Z8Yv2.5_suppressed_Harley_cleaned.xlsx", sheet = "Period 1")
...
# any_duplicated <- any(duplicated(COPD_HES$`Site Code`))
...
COPD_HES$`Provider Code` <- sub("-X$", "", COPD_HES$`Provider Code`)
...
COPD_HES <- COPD_HES %>%
  ...
  mutate(FDEs = as.integer(FDEs)) %>%
  summarise(Episodes_sum = sum(FDEs)) %>%
  left_join(COPD_HES, by = c("Provider Code", "Site Code")) %>%
...
COPD_HES$Episodes_sum[COPD_HES$`Provider Code` == "RA2" & COPD_HES$`Site Code` == "RA201"] <- 335+45
...
#--------
# Load audit data
COPD_audit <- readRDS("C:/Alex Harley/Audit_2023_onwards/2022-2023/COPD/Data/tidyData/COPD_SCC_2022-2023_clean_data.RDS")

# Count the number of episodes per hospital
COPD_audit <- COPD_audit %>%
  group_by(hosp_code) %>%
  mutate(row_count = n(), .after = trust_code) %>%
  filter(row_number() == 1)

# Check if the hospital in the dataset is unique
any_duplicated <- any(duplicated(COPD_audit$hosp_code))
print(any_duplicated)
...
COPD_audit$ODS_code[COPD_audit$ODS_code == "RRK02"] <- "RRK15"

# change Broomsfield code to match with 'mid essex hospital', which we assume is Broomsfield

COPD_audit$ODS_code[COPD_audit$ODS_code == "RDEP3"] <- "RAJ32"

# Change the ODS_code manually (according to hosp_name and change their mismatched ODS_code)
COPD_audit$ODS_code[COPD_audit$ODS_code == "RM318"] <- "R0A66"
COPD_audit$ODS_code[COPD_audit$ODS_code == "RX2N2"] <- "E0A3H"
...
# 
# Join COPD_audit_link with HES data
COPD_audit_final <- left_join(COPD_audit, COPD_HES, by = c("ODS_code" = "Site Code"))

# Check any mismatched hospital data (appear in audit but not appear in HES Data)
sum(is.na(COPD_audit_final$`Provider Code`))   # 3 is missing

# Locate which rows are missing
rows_missing <- COPD_audit_final[!complete.cases(COPD_audit_final$`Provider Code`), ]
print(rows_missing)
...
# Export as csv file
write.csv(COPD_audit_final, "C:/Alex Harley/Audit_2023_onwards/2022-2023/COPD/Analysis/Output/COPD_CA_England.csv", row.names = FALSE)
...
# Load COPD PEDW data
COPD_PEDW <- read_xlsx("C:/Alex Harley/Audit_2023_onwards/2022-2023/COPD/Data/rawData/Request_770_COPD_Harley_cleaned.xlsx", sheet = "Period_1_Data")

# Check if hospitals in the dataset is unique
any_duplicated <- any(duplicated(COPD_PEDW$`Site Code`))
print(any_duplicated)
# Great no duplicate now!

# Join audit data with linking file
COPD_audit_link <- left_join(COPD_audit, linking, by = c("ODS_code" = "ODS.Site.Code"))

# Extract the hospitals in Wales
COPD_audit_link <- COPD_audit_link %>%
  filter(country == "Wales")

# Check number of hospital not having info in linking file (Wales)
sum(is.na(COPD_audit_link$Crown.hosp_code))   # 1

# Join AA_audit_link with PEDW data
COPD_audit_final <- left_join(COPD_audit_link, COPD_PEDW, by = c("ODS_code" = "Site Code"))

# Check any mismatched hospital data (appear in audit but not matched with HES Data)
sum(is.na(COPD_audit_final$Discharges))   # 0 
...
# Export as csv file
write.csv(COPD_audit_final, "C:/Alex Harley/Audit_2023_onwards/2022-2023/COPD/Analysis/Output/COPD_CA_Wales.csv", row.names = FALSE)
